...
app.post('/api/promo/validate', async (c) => {
  ...
  if (!promo) {
    ...
    return c.json({ valid: true, code: upperCode, discount, description: 'Welcome discount', message: 'Promo applied!' })
  }
  ...
  return c.json({ valid: true, code: upperCode, discount, description: promo.description, savings: discount })
})
...
app.post('/api/auth/signup', async (c) => {
  ...
  return c.json({ success: true, message: `OTP ${otp} sent to ${phone}`, user_id: 'USR-' + Math.random().toString(36).slice(2,9).toUpperCase(), verify_required: true }, 201)
})
...
app.post('/api/promos', async (c) => {
  ...
  return c.json({ success: true, promo: { code: code.toUpperCase(), type, value: Number(value), max_uses: Number(max_uses) || 1000, used: 0, expires: expires || null, categories: categories || ['all'], status: 'active', created_at: new Date().toISOString() }, message: `Promo code ${code.toUpperCase()} created successfully!` }, 201)
})
...
app.post('/api/admin/events/:id/approve', async (c) => {
  ...
  return c.json({ success: true, event_id: id, status: approved ? 'approved' : 'rejected', action, reason: reason || null, message: approved ? `Event ${id} approved! Organiser notified via WhatsApp.` : `Event ${id} rejected. Reason sent to organiser.`, notified_at: new Date().toISOString() }, 201)
})
...
app.post('/api/organiser/team', async (c) => {
  ...
  return c.json({ success: true, team_member: { id: 'TM-' + Date.now(), name, email, role, permissions: permissions || [], status: 'invited', invited_at: new Date().toISOString() }, message: `Invite sent to ${email}. They can join using the link in their email.` }, 201)
})
...
app.post('/api/merch/order', async (c) => {
  ...
  return c.json({ success: true, order: { id: orderId, user_id: body.user_id || 'USR-001', items, subtotal, shipping, gst, total: subtotal + shipping + gst, estimated_delivery: '3-5 business days', tracking_enabled: true, payment_method: body.payment_method || 'upi', status: 'confirmed', confirmed_at: new Date().toISOString() }, message: `Order ${orderId} confirmed! Estimated delivery in 3-5 business days.` }, 201)
})
...
app.post('/api/whitelabel/provision', async (c) => {
  ...
  return c.json({ success: true, instance: { id: instanceId, venue_name, domain: domain || `${venue_name.toLowerCase().replace(/\s+/g,'-')}.indtix.com`, city, contact_email, plan: plan || 'starter', status: 'provisioning', features: ['Custom branding', 'Event management', 'Ticket sales', 'Analytics', 'WhatsApp notifications'], monthly_fee: plan === 'enterprise' ? 49999 : plan === 'pro' ? 14999 : 4999, provisioned_at: new Date().toISOString(), live_in: '24-48 hours' }, message: `White-label instance ${instanceId} provisioning. Live in 24-48 hours!` }, 201)
})
...
app.post('/api/loyalty/redeem', async (c) => {
  ...
  return c.json({ success: true, user_id, points_redeemed: points, cashback_added: cashback, new_balance: 1240 - points, wallet_credited: cashback, message: `${points} points redeemed → ₹${cashback} added to your wallet!` })
})
...
app.post('/api/events/:id/clone', async (c) => {
  ...
  return c.json({ success: true, cloned_event_id: newId, source_event_id: id, new_title: new_title || 'Copy of event', new_date: new_date || '2026-07-01', new_city: new_city || 'Mumbai', status: 'draft', message: `Event cloned as ${newId}. Edit it in your dashboard.` }, 201)
})

// PHASE 4: Organiser Coupon Code Distribution
app.post('/api/organiser/coupons/distribute', async (c) => {
  const { event_id, coupon_code, audience, channel } = await c.req.json().catch(() => ({}))
  if (!event_id || !coupon_code) return c.json({ error: 'event_id and coupon_code required' }, 400)
  const count = channel === 'whatsapp' ? 2841 : channel === 'email' ? 4200 : 1200
  return c.json({ success: true, distributed: count, coupon_code, event_id, channel: channel || 'whatsapp', estimated_redemption: Math.round(count * 0.12), message: `Coupon ${coupon_code} distributed to ${count} ${audience || 'attendees'}.` }, 201)
})
...
app.post('/api/events/:id/emergency', async (c) => {
  const id = c.req.param('id')
  const { type, message, zones, action_required } = await c.req.json().catch(() => ({}))
  if (!type || !message) return c.json({ error: 'type and message required' }, 400)
  const emergencyId = 'EMR-' + Math.random().toString(36).slice(2,7).toUpperCase()
  return c.json({ success: true, emergency_id: emergencyId, event_id: id, type: type || 'general', message, broadcast_to: zones || 'all', attendees_notified: 4180, channels: ['push', 'wristband_led', 'pa_system'], led_color: type === 'fire' ? '#FF0000' : type === 'medical' ? '#00FF00' : '#FF8800', dispatched_at: new Date().toISOString(), status: 'broadcast_sent' }, 201)
})
...
app.post('/api/promos/bulk-create', async (c) => {
  ...
  return c.json({ success: true, created: codes.length, codes, created_at: new Date().toISOString() }, 201)
})
...
// ─── MISSING ROUTES (QA FIXES) ──────────────────────────────

// GET /api/organiser/events — list organiser's events
app.get('/api/organiser/events', (c) => {
  return c.json({
    events: [
      { id: 'e1', title: 'Diljit Dosanjh – Dil-Luminati Tour', status: 'live', city: 'Mumbai', date: '2026-04-12', sold: 3210, capacity: 5000, revenue: 4820000 },
      { id: 'e2', title: 'Sunburn Arena – Mumbai', status: 'live', city: 'Mumbai', date: '2026-04-26', sold: 1420, capacity: 3000, revenue: 2840000 },
      { id: 'EVT-PENDING-001', title: 'Arijit Singh Live', status: 'pending_review', city: 'Mumbai', date: '2026-05-15', sold: 0, capacity: 8000, revenue: 0 }
    ],
    total: 3,
    stats: { total_revenue: 7660000, total_sold: 4630 }
  })
})
...
app.get('/api/sponsors', (c) => {
  return c.json({
    sponsors: [
      { id: 'SP-001', name: 'Pepsi India', tier: 'title', event_id: 'e1', budget: 5000000, status: 'active' },
      { id: 'SP-002', name: 'Red Bull', tier: 'associate', event_id: 'e2', budget: 2000000, status: 'active' },
      { id: 'SP-003', name: 'Swiggy', tier: 'digital', event_id: 'e1', budget: 1500000, status: 'active' }
    ],
    total: 3
  })
})
...
app.get('/api/status', (c) => {
  return c.json({
    status: 'operational',
    platform: 'INDTIX',
    last_incident: '2026-02-15T18:30:00Z',
    uptime_30d: 99.94,
    services: [
...
// GET /api/merch/:id/order — get merch order status  
app.get('/api/merch/order/:id', (c) => {
  const id = c.req.param('id')
  return c.json({ order: { id, status: 'dispatched', tracking: 'TRK' + id, updated_at: new Date().toISOString() } })
})
